...
if indo_voice_id:
    ...
    print("⚠️ Voice bahasa Indonesia tidak ditemukan, menggunakan default voice.")

tts_lock = threading.Lock()

def speak_text(text):
    """TTS stabil & cepat — hanya 1 engine global"""
    def run():
        with tts_lock:
            engine.say(text)
            engine.runAndWait()
...
is_processing = False  # untuk menghindari prediksi ganda
...
def callback(recognizer, audio):
    ...
    try:
        ...
        print(f"📊 Prediksi: {result['label']} (Prob: {result['prob']})")
        print(f"💬 Tanggapan: {result['respon']}")
...
if __name__ == "__main__":
    ...
    # Batasi waktu input agar tidak delay
